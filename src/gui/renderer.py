...
class GUIRenderer:
    ...
    def _draw_legal_moves(self, screen: pygame.Surface, moves: Set[Tuple[int, int]]):
        for move in moves:
            ...
            pygame.draw.circle(screen, self.COLORS["legal_moves"], center, self.square_size // 8)

    def _draw_last_move_highlight(self, screen: pygame.Surface, last_move: Tuple[Tuple[int, int], Tuple[int, int]]):
    ...
    def _draw_piece(self, screen: pygame.Surface, piece: Piece, pos: Tuple[int, int]):
        text_color = self.COLORS["white_piece"] if piece.is_white else self.COLORS["black_piece"]
    ...
    def _draw_game_over(self, screen: pygame.Surface, state: GameState):
        ...
        text = self.game_over_font.render(texts[state.game_result], True, self.COLORS["text"])
        text_rect = text.get_rect(center=(self.screen_width//2, self.screen_height//2))
        screen.blit(text, text_rect)

        # Play again prompt
        prompt = self.info_font.render("Press ESC to play again", True, self.COLORS["text"])
